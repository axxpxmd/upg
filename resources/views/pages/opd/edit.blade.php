...
@section('content')
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white mb-2">Edit OPD</h1>
                <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">Perbarui informasi OPD {{ $opd->n_opd }}</p>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <form action="{{ route('opd.update', $opd) }}" method="POST" class="p-6 sm:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="n_opd" name="n_opd" value="{{ old('n_opd', $opd->n_opd) }}" required placeholder="Contoh: Dinas Komunikasi dan Informatika"
                    ...
                    <div>
                        ...
                        <textarea id="alamat" name="alamat" rows="4" placeholder="Contoh: Jl. Maruga No. 1, Pondok Aren, Tangerang Selatan"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0093FF] transition-all resize-none @error('alamat') border-red-500 @enderror">{{ old('alamat', $opd->alamat) }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
